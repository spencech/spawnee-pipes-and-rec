 ...
 */
export function parseGateOutput(gate: GateType, stdout: string, stderr: string): IGateFailure[] {
	try {
		switch (gate) {
			case 'typecheck':
 ...
 */
function parseTypecheckOutput(stdout: string, stderr: string): IGateFailure[] {
	const output = stdout + '\n' + stderr;
	const failures: IGateFailure[] = [];
	...
	while ((match = colonPattern.exec(output)) !== null) {
		...
		const alreadyCaptured = failures.some(f =>
			f.file === match![1] && f.line === parseInt(match![2], 10)
		);
		if (!alreadyCaptured) {
			failures.push({
				gate: 'typecheck',
 ...
 */
function parseUnitOutput(stdout: string, stderr: string): IGateFailure[] {
	const output = stdout + '\n' + stderr;
	const failures: IGateFailure[] = [];
 ...
 */
function parseE2eOutput(stdout: string, stderr: string): IGateFailure[] {
	const output = stdout + '\n' + stderr;
	const failures: IGateFailure[] = [];
 ...
 * Falls back to regex parsing if JSON parsing fails.
 */
function parseLintOutput(stdout: string, stderr: string): IGateFailure[] {
	const failures: IGateFailure[] = [];
...
function createCatchAllFailure(gate: GateType, stdout: string, stderr: string): IGateFailure {
...
function extractSurroundingContext(text: string, index: number, chars: number): string {
	const start = Math.max(0, index - 100);
	const end = Math.min(text.length, index + chars);
	return text.substring(start, end);
}

function deduplicateFailures(failures: IGateFailure[]): IGateFailure[] {
