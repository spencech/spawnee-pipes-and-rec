...
import { IGateConfig, IGateResult, GateType } from './types.js';
...
async function runAutomatedGate(
	config: IGateConfig,
	options: IGateRunnerOptions
): Promise<IGateResult> {
...
async function runManualGate(
	config: IGateConfig,
	_options: IGateRunnerOptions
): Promise<IGateResult> {
	...
	const { approved, feedback } = await inquirer.prompt<{ approved: boolean; feedback: string }>([
		{
			type: 'confirm',
			name: 'approved',
 ...
 */
function buildCommandArgs(config: IGateConfig): string[] {
	const command = config.command!;
