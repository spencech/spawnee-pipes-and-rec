...
import { PipelineController, IPipelineResult } from './core/pipeline-controller.js';
...
import { IGateConfig, IGateResult } from './validation/types.js';
...
program
  ...
  .action(async (templatePath: string, options: RunOptions, cmd) => {
    ...
    if (options.validate || options.artifact) {
      ...
      const result: IPipelineResult = await pipeline.run();
...
program
  ...
  .action(async (templatePath: string, options: { repoDir?: string }) => {
    if (!existsSync(templatePath)) {
      console.error(chalk.red(`Error: Template file not found: ${templatePath}`));
      process.exit(1);
    }
    ...
    const gates = template.validation_strategy as IGateConfig[];
    if (gates.length === 0) {
    ...
    runner.on('gateStarted', (config: IGateConfig) => {
      console.log(chalk.blue(`   Running: ${config.gate}`));
    });
    runner.on('gatePassed', (result: IGateResult) => {
      console.log(chalk.green(`   ✓ Passed: ${result.gate} (${result.durationMs}ms)`));
    });
    runner.on('gateFailed', (result: IGateResult) => {
